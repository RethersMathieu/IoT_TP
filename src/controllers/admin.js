...
module.exports = function (dbo) {
    return {
        ...
        acceptNewUser: (req, res) => {
            ...
            dbo.collection('in-waiting').findOne({ _id: new ObjectId(user_id) }, function (err, newUser) {
                if (err) return res.status(500).json({ error: 'Erreur inatandue.' });
        ...
        declineNewUser: (req, res) => {
            const { user_id } = req.body;
            dbo.collection('in-waiting').deleteOne({ _id: new ObjectId(user_id) }, function (err, userDeleted) {
                if (err) return res.status(500).json({ error: 'Erreur inatandue.' });
